...
const StageTag12 = "proxyutil-stage12"
...
func (s *Stage12) JSON(v any) string { b, _ := json.Marshal(v); return string(b) }
func (s *Stage12) Sha256(b []byte) string { sum := sha256.Sum256(b); return hex.EncodeToString(sum[:]) }
func (s *Stage12) Join(parts ...string) string { return strings.Join(parts, "-") }
func (s *Stage12) Truncate(b []byte, n int) []byte {
	if len(b) <= n { return append([]byte(nil), b...) }
	return append([]byte(nil), b[:n]...)
}
func (s *Stage12) Window(start time.Time, d time.Duration) bool { return time.Since(start) < d }
func (s *Stage12) Dup(b []byte) []byte { return append([]byte(nil), b...) }
func (s *Stage12) Prefix(b []byte, p string) bool { return bytes.HasPrefix(b, []byte(p)) }
func (s *Stage12) Tag() string { return fmt.Sprintf("stage-%d-%d", 12, s.n) }
func (s *Stage12) Reset() { s.mu.Lock(); s.n = 0; s.buf = s.buf[:0]; s.mu.Unlock() }
